module.exports = function(app) {
  ...
  widgets.cmenu = function (menuData) {
    var menuOutput = "";
    if (!menuData['items'] || !(menuData['items'] instanceof Array)) {
      menuOutput += (menuOutput) ? helpers.end_tag('ul') : "";
      return menuOutput;
    }
    ...
    for(var i = 0; i < menuItems.length; i++) {
      ...
      var linkSuffix = menuItems[i]['linkSuffix'] || "";
      if (menuItems[i]['items'] && (menuItems[i]['items'] instanceof Array) && menuItems[i].length > 0) {
        linkSuffix += menuItems[i]['caret'] || '<span class="caret"></span>';
      }
      var iconType = menuItems[i]['iconType'] || "glyphicon";
      var linkPrefix = (menuItems[i]['linkPrefix'] || "") + (menuItems[i]['icon'] ? '<i class="'+iconType+' '+iconType+'-'+menuItems[i]['icon']+'"></i>' : "");
      menuOutput += helpers.link_to(linkPrefix+menuItems[i]['label']+linkSuffix, menuItems[i]['url'], menuItems[i]['linkHtmlOptions'] || {});
      menuOutput += widgets.cmenu(menuItems[i]);
      menuOutput += helpers.end_tag("li");
    }
    menuOutput += (menuOutput) ? helpers.end_tag('ul') : "";
    return menuOutput;
  };
